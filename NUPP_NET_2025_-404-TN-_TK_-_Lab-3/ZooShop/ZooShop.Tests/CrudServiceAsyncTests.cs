...
using ZooShop.Common;
using Xunit;

namespace ZooShop.Tests
{
    public class CrudServiceAsyncTests : IDisposable
    {
        ...
        {
            var service = new CrudServiceAsync<Product>(_tempFile);

            var d1 = Product.GenerateSample();
            ...
            // save
            await service.CreateAsync(Product.GenerateSample());
        ...
        {
            var service = new CrudServiceAsync<Product>(_tempFile);
            for (int i = 0; i < 25; i++) await service.CreateAsync(Product.GenerateSample());
        ...
        {
            var service = new CrudServiceAsync<Product>(_tempFile);
            var tasks = Enumerable.Range(0, 200).Select(i => service.CreateAsync(Product.GenerateSample())).ToArray();
